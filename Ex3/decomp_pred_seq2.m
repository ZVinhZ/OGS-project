function [ U, V, Lam, Mu, k ] = decomp_pred_seq2( A,b,C_e,d_e,C_i,d_i, i0, lam0,mu0, beta, tol, rho_uzawa,tol_uzawa )
%% Algorthime de décompostion par prédiction en version séquentielle (on ne donne qu'un prix de départ)
...
% Cas 3 : s'il y a des contraintes d'égalité et d'inégalité
  ...
  V(:,1) = [d_e - (C_e*U(:,1) - C_e(:,i0)*U(i0,1)) ; d_i - (C_i*U(:,1) - C_i(:,i0)*U(i0,1))];
  ...
  while err > tol && k < iter_max
      ...
      V(:,k) = [d_e - (C_e*U(:,k) - C_e(:,i0)*U(i0,k)) ; d_i - (C_i*U(:,k) - C_i(:,i0)*U(i0,k))];

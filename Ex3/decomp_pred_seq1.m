function [ U, V, Lam, Mu, k ] = decomp_pred_seq1( A,b,C_e,d_e,C_i,d_i, i0, v0, gamma, tol, rho_uzawa,tol_uzawa )
%% Algorthime de décompostion par prédiction en version séquentielle (on ne donne qu'une allocation de départ)
...
V = v0;
Lam = zeros(length(d_e),1); % en colonne, les multiplicateurs de Lagrange associés aux N sous-problèmes
Mu = zeros(length(d_i),1);
...
if C_i == 0
  ...
  %  i) Unité i0
  [U(i0,1),Lam(:,1),~,~] = Uzawa(A(i0,i0),b(i0),C_e(:,i0),V(:,1),0,0, Lam(:,1),0, rho_uzawa,tol_uzawa);
  ...
  while err > tol && k < iter_max
      ...
      % i) Unité i0
      [U(i0,k),Lam(:,k),~,~] = Uzawa(A(i0,i0),b(i0),C_e(:,i0),V(:,k),0,0, Lam(:,k-1),0, rho_uzawa,tol_uzawa);
...
elseif C_e == 0 
  ...
  %  i) Unité i0
  [U(i0,1),~,Mu(:,1),~] = Uzawa(A(i0,i0),b(i0),0,0,C_i(:,i0),V(:,1), 0,Mu(:,1), rho_uzawa,tol_uzawa);
  ...
  while err > tol && k < iter_max
      ...
      % i) Unité i0
      [U(i0,k),~,Mu(:,k),~] = Uzawa(A(i0,i0),b(i0),0,0,C_i(:,i0),V(:,k), 0,Mu(:,k-1), rho_uzawa,tol_uzawa);
...
% Cas 3 : s'il y a des contraintes d'égalité et d'inégalité
  ...
  % Première itération (k = 1)
  %  i) Unité i0
  [U(i0,1),Lam(:,1),Mu(:,1),~] = Uzawa(A(i0,i0),b(i0),C_e(:,i0),V(1:nb_ega,1),C_i(:,i0),V((nb_ega+1):m,1), Lam(:,1),Mu(:,1), rho_uzawa,tol_uzawa);
  ...
  while err > tol && k < iter_max
      ...
      %  i) Unité i0
      [U(i0,k),Lam(:,k),Mu(:,k),~] = Uzawa(A(i0,i0),b(i0),C_e(:,i0),V(1:nb_ega,k),C_i(:,i0),V((nb_ega+1):m,k), Lam(:,k-1),Mu(:,k-1), rho_uzawa,tol_uzawa);
